...
带来的新问题：如果已提交的事务的数据未写入磁盘，服务器宕机了，这一部分数据就丢失了。这违背了事务的持久性。

解决这个问题，我们也需要日志，在事务提交时，将事务中修改过的页写入日志（redo log），日志写入成功再释放锁。有了日志后，即使服务器再发生崩溃，也可以保证数据不丢失，重启后也可以从日志中取出这部分记录，重新做一遍刷盘，便可将数据恢复。

写页数据数据要写入磁盘，写日志文件同样要写入磁盘，何必多此一举呢？

写日志是顺序写，而写数据本身是随机写。顺序写效率会更高，由于实现的是页粒度的读写，这一点可能相差不太明显，在行粒度的读写机制后，差距就变的很大了。这就是 **WAL** 技术(**Write-Ahead Logging**)，数据库的设计中普遍。
